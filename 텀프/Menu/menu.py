...
class Menu():
    def __init__(self, game):
        ...
        self.offset = -100 # 시작 커서의 위치를 조정해주는 변수.
...
class MainMenu(Menu):
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.game.draw_text(f"W / {u'\u2191'} : move to cursor up", 20, self.menual_x, self.menual_y)
            self.game.draw_text(f"S / ↓ : move to cursor down", 20, self.menual_x, self.menual_y+20)
            self.game.draw_text(f"Enter / ESC : back to previous Menu", 20, self.menual_x, self.menual_y+40)
            self.draw_cursor()
            self.blit_screen()
    ...
    def check_input(self):
        ...
        if self.game.START_KEY: # enter키가 눌리면?
